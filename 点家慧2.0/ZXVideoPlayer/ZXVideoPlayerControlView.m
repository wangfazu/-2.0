...
{
    if (!_playButton) {
        _playButton = [UIButton buttonWithType:UIButtonTypeCustom];//@"kr-video-player-play"
...
{
    if (!_pauseButton) {
        _pauseButton = [UIButton buttonWithType:UIButtonTypeCustom];//suspend//@"kr-video-player-pause"
        [_pauseButton setImage:[UIImage imageNamed:@"suspend"] forState:UIControlStateNormal];
...
- (UIButton *)fullScreenButton
{
    if (!_fullScreenButton) {
        _fullScreenButton = [UIButton buttonWithType:UIButtonTypeCustom];//kr-video-player-fullscreen
        [_fullScreenButton setImage:[UIImage imageNamed:@"amplification"] forState:UIControlStateNormal];
...
{
    if (!_progressSlider) {
        _progressSlider = [[UISlider alloc] init];//audio_file_button
        [_progressSlider setThumbImage:[UIImage imageNamed:@"audio_file_button"] forState:UIControlStateNormal];//@"kr-video-player-point"
        [_progressSlider setMinimumTrackTintColor:UIColorFromRGB(0x43d3a2)];
...
{
    if (!_backButton) {
        _backButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, kVideoControlBarHeight , kVideoControlBarHeight)];//return&@"zx-video-banner-back"
...
{
    if (!_lockButton) {
        _lockButton =[[UIButton alloc] initWithFrame:CGRectMake(0, 0, kVideoControlBarHeight, kVideoControlBarHeight)];//@"zx-video-player-unlock"
        [_lockButton setImage:[UIImage imageNamed:@"share"] forState:UIControlStateNormal];
...
//        [_lockButton setImage:[UIImage imageNamed:@"zx-video-player-lock"] forState:UIControlStateSelected];
        ...
        [_lockButton addTarget:self action:@selector(shareVideoClick) forControlEvents:UIControlEventTouchUpInside];
    }
    return _lockButton;
}
